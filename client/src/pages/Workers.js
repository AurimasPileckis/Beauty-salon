...
const Workers = () => {
    ...
    return(
        ...
        <select onChange={handleSorting}>
            ...
            <option value='2'>Mažėjancia tvarka</option>
        </select>
        {workers && workers.map(worker =>
            <div key={worker.id}>
                <img src={worker.photo} style={{ maxWidth: "150px", marginTop: "20px"}} />
                <div>{worker.first_name + ' ' + worker.last_name}</div>
                <div>{worker.saloon.name}</div>
                <div>Įvertinimas: {parseFloat(worker.total_rating).toFixed(1)}</div>
            </div>
            )}
        </>
    )
